...
@section('page-title', 'Target Domains')
...
<div class="page-enter">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Target Domains</h2>
            <p class="text-slate-500 mt-1">Manage domains for campaign link distribution</p>
        </div>
        <div class="flex flex-col sm:flex-row gap-3 sm:items-center">
            <form method="GET" action="{{ route('campaign-domains.index') }}" class="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                @if($statusFilter !== 'all')
                ...
                @if(($search ?? '') !== '')
                    <a href="{{ route('campaign-domains.index', $statusFilter !== 'all' ? ['status' => $statusFilter] : []) }}" class="text-xs text-slate-500 hover:text-slate-700">Clear</a>
                @endif
            </form>
            <form action="{{ route('campaign-domains.check-all', $listQuery) }}" method="POST" class="inline w-full sm:w-auto">
                @csrf
                <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-emerald-300 rounded-lg text-sm font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 transition-colors">
            ...
            <form action="{{ route('campaign-domains.import') }}" method="POST" enctype="multipart/form-data" class="inline w-full sm:w-auto">
                @csrf
            ...
            <a href="{{ route('campaign-domains.export', $listQuery) }}" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button type="button" onclick="openAddDomainModal()" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <form method="POST" action="{{ route('campaign-domains.bulk-destroy') }}" id="bulk-delete-form" class="hidden mb-4 flex items-center gap-3 px-4 py-3 bg-slate-50 rounded-xl border border-slate-200" onsubmit="return confirm('Delete selected target domains?');">
        @csrf
    ...
    <div class="flex flex-col lg:flex-row gap-4 mb-6 w-full">
        <div class="flex-1 min-w-0 w-full bg-white rounded-xl border border-purple-200 p-4 shadow-sm">
            <p class="text-sm text-slate-500">Connected</p>
            <p class="text-2xl font-bold text-purple-600 mt-1">{{ number_format($statusCounts['active'] ?? 0) }}</p>
            ...
            <div class="inline-flex flex-wrap rounded-xl border border-slate-200 bg-slate-100/80 p-1 gap-1" role="tablist">
                @php
                    $filterTabs = [
                        ...
                        'active' => ['label' => 'Connected', 'count' => $statusCounts['active'] ?? 0, 'activeClass' => 'bg-purple-600 text-white'],
                ...
                @foreach($filterTabs as $key => $tab)
                    ...
                    @endphp
                    <a href="{{ route('campaign-domains.index', $tabQuery) }}"
                ...
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider w-12">
                            <input type="checkbox" id="select-all" class="rounded border-slate-300 text-purple-600 focus:ring-purple-500" title="Select all">
                        </th>
                ...
                <tbody class="divide-y divide-slate-200" id="domains-table-body">
                    @forelse($domains ?? [] as $domain)
                        <tr class="hover:bg-slate-50/50 transition-colors domain-row" data-search="{{ strtolower($domain->domain . ' ' . ($domain->api_url ?? '') . ' ' . ($domain->status ?? '') . ' ' . ($domain->notes ?? '')) }}">
                            <td class="px-6 py-4">
                                <input type="checkbox" form="bulk-delete-form" name="domain_ids[]" value="{{ $domain->id }}" class="domain-checkbox rounded border-slate-300 text-purple-600 focus:ring-purple-500">
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <form method="POST" action="{{ route('campaign-domains.check', $domain) }}" class="inline" title="Check connection">
                                        @csrf
                                        <button type="submit" class="p-2 rounded-lg text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 transition-colors">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        </button>
                                    </form>
                                    <a href="{{ route('campaign-domains.edit', $domain) }}" title="Edit" class="p-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-purple-600 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('campaign-domains.destroy', $domain) }}" class="inline" onsubmit="return confirm('Delete this target domain?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-500">
                                ...
                                <p class="mt-2">No target domains yet. Add manually to get started.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @foreach($imports as $imp)
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-sm text-slate-600">{{ $imp->campaign_domains_count ?? 0 }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-500">{{ $imp->created_at?->format('M d, Y H:i') ?? '-' }}</td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="inline-flex items-center gap-1 justify-end">
                                        @if(($imp->campaign_domains_count ?? 0) > 0)
                                            <form method="POST" action="{{ route('campaign-domains.imports.destroy-domains', $imp) }}" class="inline" onsubmit="return confirm('Delete {{ $imp->campaign_domains_count }} domain(s) imported from this file? This cannot be undone.');">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @endif
                                        <form method="POST" action="{{ route('campaign-domains.imports.destroy', $imp) }}" class="inline" onsubmit="return confirm('Delete only this import record{{ ($imp->campaign_domains_count ?? 0) > 0 ? ' (domains will stay in your list)' : '' }}?');">
                                            @csrf
                                            @method('DELETE')
...
<div id="add-domain-modal" class="hidden fixed inset-0 z-[100] overflow-y-auto" role="dialog" aria-modal="true">
    ...
    <div class="relative flex min-h-full items-center justify-center p-4">
        <div class="relative bg-white rounded-xl shadow-lg max-w-md w-full p-6 z-10">
            <h3 class="text-lg font-semibold text-slate-800 mb-4">Add Target Domain</h3>
            <form method="POST" action="{{ route('campaign-domains.store') }}">
                @csrf
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Domain</label>
                        <input type="text" name="domain" required placeholder="example.com" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">API URL</label>
                        <input type="url" name="api_url" required placeholder="http://example.com/api (https also OK)" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                        <p class="text-xs text-slate-500 mt-1">Use http:// if the site has no SSL.</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">API Key (optional)</label>
                        <input type="text" name="api_key" placeholder="Your API key" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Notes (optional)</label>
                        <textarea name="notes" rows="2" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-purple-500 focus:ring-purple-500"></textarea>
                    </div>
                </div>
                <div class="flex gap-3 mt-6">
                    ...
                    <button type="submit" class="flex-1 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Add</button>
                </div>
            </form>
        </div>
    </div>
