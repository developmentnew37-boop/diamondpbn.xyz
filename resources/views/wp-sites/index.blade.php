...
@section('title', 'WP Sites')
@section('page-title', 'WP Sites')
...
<div class="page-enter">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">WP Sites</h2>
            <p class="text-slate-500 mt-1">Manage WordPress sites and their hidden link API endpoints</p>
        </div>
        <div class="flex flex-col sm:flex-row gap-3 sm:items-center">
            <form method="GET" action="{{ route('wp-sites.index') }}" class="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                ...
                <div class="relative">
                    <input
                        ...
                        name="search"
                        value="{{ $search ?? '' }}"
                        placeholder="Search sites or API URL..."
            ...
            <form action="{{ route('wp-sites.check-all', $listQuery) }}" method="POST" class="inline w-full sm:w-auto">
                @csrf
                <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-sky-300 rounded-lg text-sm font-medium text-sky-700 bg-sky-50 hover:bg-sky-100 transition-colors" title="Queue health checks for all WP sites (or current search results)">
            ...
            <button type="button" onclick="openAddWpSiteModal()" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-sky-600 text-white rounded-lg text-sm font-medium hover:bg-sky-700 transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex flex-col lg:flex-row gap-4 mb-6 w-full">
        <div class="flex-1 min-w-0 w-full bg-white rounded-xl border border-sky-200 p-4 shadow-sm">
            ...
            <p class="text-2xl font-bold text-sky-600 mt-1">{{ number_format($statusCounts['active'] ?? 0) }}</p>
                ...
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider w-12">
                            <input type="checkbox" id="select-all" class="rounded border-slate-300 text-sky-600 focus:ring-sky-500" title="Select all">
                        </th>
                    ...
                    @forelse($wpSites ?? [] as $wpSite)
                        <tr class="hover:bg-slate-50/50 transition-colors wp-site-row" data-search="{{ strtolower($wpSite->domain . ' ' . ($wpSite->api_url ?? '') . ' ' . ($wpSite->status ?? '') . ' ' . ($wpSite->notes ?? '')) }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $status = $wpSite->status ?? 'inactive';
                                    $label = $status === 'inactive' ? 'Not connected' : ucfirst($status);
                                @endphp
                                <span class="px-2.5 py-0.5 text-xs font-medium rounded-full {{ $status === 'active' ? 'bg-sky-100 text-sky-700' : ($status === 'error' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600') }}">{{ $label }}</span>
                            </td>
                            <td class="hidden lg:table-cell px-6 py-4 text-sm text-slate-500">{{ $wpSite->last_checked_at ? $wpSite->last_checked_at->diffForHumans() : '-' }}</td>
                            <td class="hidden lg:table-cell px-6 py-4 text-sm max-w-xs" title="{{ $wpSite->last_health_error ?? '' }}">
                                @if($wpSite->last_health_error)
                                    <span class="text-red-600">{{ Str::limit($wpSite->last_health_error, 50) }}</span>
                                @elseif(($wpSite->status ?? '') === 'error')
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <form method="POST" action="{{ route('wp-sites.recheck', $wpSite) }}" class="inline" title="Re-check health">
                                        @csrf
                                        <button type="submit" class="p-2 rounded-lg text-slate-500 hover:bg-sky-50 hover:text-sky-600 transition-colors" title="Re-check health">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                        </button>
                                    </form>
                                    <a href="{{ route('wp-sites.edit', $wpSite) }}" title="Edit" class="p-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-sky-600 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('wp-sites.destroy', $wpSite) }}" class="inline" onsubmit="return confirm('Delete this WP site? All related data will be removed.');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center text-slate-500">
                                ...
                                <p class="mt-2">No WP sites yet. Import from Excel or add manually.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @if(isset($imports) && $imports->isNotEmpty())
        <div class="mt-8">
            ...
            <p class="text-sm text-slate-500 mb-3">Deleting an import file does <strong>not</strong> remove the WP sites it added. Use <strong>Delete imported sites</strong> to remove those sites from your list.</p>
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Linked sites</th>
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @foreach($imports as $imp)
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2.5 py-0.5 text-xs font-medium rounded-full {{ $imp->status === 'completed' ? 'bg-sky-100 text-sky-700' : ($imp->status === 'failed' ? 'bg-red-100 text-red-700' : ($imp->status === 'processing' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600')) }}">{{ ucfirst($imp->status ?? 'pending') }}</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-slate-600">{{ $imp->total_rows ?? 0 }} / {{ $imp->imported_count ?? 0 }} / {{ $imp->skipped_count ?? 0 }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-600">{{ $imp->wp_sites_count ?? 0 }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-500">{{ $imp->created_at?->format('M d, Y H:i') ?? '-' }}</td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="inline-flex items-center gap-1 justify-end">
                                        @if(($imp->wp_sites_count ?? 0) > 0)
                                            <form method="POST" action="{{ route('wp-sites.imports.destroy-sites', $imp) }}" class="inline" onsubmit="return confirm('Delete {{ $imp->wp_sites_count }} WP site(s) imported from this file? This cannot be undone.');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" title="Delete imported sites" class="px-2 py-1 text-xs font-medium rounded-lg bg-red-50 text-red-700 hover:bg-red-100">Delete imported sites</button>
                                            </form>
                                        @endif
                                        <form method="POST" action="{{ route('wp-sites.imports.destroy', $imp) }}" class="inline" onsubmit="return confirm('Delete only this import record{{ ($imp->wp_sites_count ?? 0) > 0 ? ' (sites will stay in your list)' : '' }}?');">
                                            @csrf
                                            @method('DELETE')
...
@push('modals')
<div id="add-wp-site-modal" class="hidden fixed inset-0 z-[100] overflow-y-auto" role="dialog" aria-modal="true">
    <div class="fixed inset-0 bg-slate-900/60" onclick="document.getElementById('add-wp-site-modal').classList.add('hidden'); document.body.classList.remove('overflow-hidden');"></div>
    <div class="relative flex min-h-full items-center justify-center p-4">
        <div class="relative bg-white rounded-xl shadow-lg max-w-md w-full p-6 z-10">
            <h3 class="text-lg font-semibold text-slate-800 mb-4">Add WP Site</h3>
            <form method="POST" action="{{ route('wp-sites.store') }}">
                @csrf
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Domain</label>
                        <input type="text" name="domain" required placeholder="example.com" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-sky-500 focus:ring-sky-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">API URL</label>
                        <input type="url" name="api_url" required placeholder="http://example.com/wp-json/pbn-hidden-link-manager/v1" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-sky-500 focus:ring-sky-500">
                ...
                <div class="flex gap-3 mt-6">
                    <button type="button" onclick="document.getElementById('add-wp-site-modal').classList.add('hidden'); document.body.classList.remove('overflow-hidden');" class="flex-1 px-4 py-2 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50">Cancel</button>
                    <button type="submit" class="flex-1 px-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700">Add</button>
                </div>
            </form>
        </div>
    </div>
...
(function() {
    const input = document.getElementById('wp-site-search');
    if (!input) return;
    const rows = Array.from(document.querySelectorAll('#wp-sites-table-body .wp-site-row'));
