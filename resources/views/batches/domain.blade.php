...
@section('title', $domain->domain . ' - ' . ($batch->name ?? 'Batch'))
@section('page-title', $domain->domain)

@section('content')
<div class="page-enter">
    <div class="mb-6">
        <a href="{{ route('batches.show', $batch) }}" class="text-sm text-slate-500 hover:text-emerald-600 mb-2 inline-block">← Back to Batch</a>
        <div class="flex flex-wrap items-center gap-4">
            <h2 class="text-2xl font-bold text-slate-800">{{ $domain->domain }}</h2>
            <span class="text-slate-500">in batch "{{ $batch->name }}"</span>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200">
            ...
            <p class="text-sm text-slate-500 mt-0.5">URL, keyword, status per link for this domain</p>
        </div>
                    ...
                    @foreach($linksWithStatus ?? [] as $i => $link)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-3 text-right">
                                ...
                                <form method="POST" action="{{ route('batches.links.destroy', [$batch, ($batchLinks[$i])]) }}" class="inline" onsubmit="return confirm('Delete this link from the batch and remove it from all remote sites?');">
                                    @csrf
                                    @method('DELETE')
