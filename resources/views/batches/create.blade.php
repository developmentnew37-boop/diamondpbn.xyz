...
@section('content')
    ...
    <div class="page-enter max-w-8xl" x-data="{ step: {{ $initialStep }} }">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-slate-800">New Link Batch</h2>
            <p class="text-slate-500 mt-1">Create a named batch and post links across your PBN sites</p>
        </div>
        <div class="mb-6 overflow-x-auto">
            <div class="flex flex-nowrap items-center gap-2 sm:gap-0 min-w-max">
                <div class="flex items-center">
                    <div :class="step >= 1 ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-500'"
                ...
                <div class="flex items-center">
                    <div :class="step >= 2 ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-500'"
                ...
                <div class="flex items-center">
                    <div :class="step >= 3 ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-500'"
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 md:p-8 w-full">
            <form method="POST" action="{{ route('batches.store') }}" id="batch-form">
                    ...
                    <div class="space-y-4 w-full max-w-8xl">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Campaign Jan 2025" required
                                class="w-full rounded-lg border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 {{ $errors->has('name') ? 'border-red-500' : '' }}">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3" placeholder="Optional notes..."
                                class="w-full rounded-lg border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </div>
                <div x-show="step === 2" x-transition:enter="transition ease-out duration-200"
                    ...
                    <p class="text-slate-500 text-sm mb-4">Choose which PBN sites to post links to. You can either select from the list
                        or paste a list of domains and we will match them to the domains in your account.</p>
                    ...
                    <div class="flex items-center gap-2 mb-4 border-b border-slate-200">
                        <button type="button"
                            ...
                            :class="domainMode === 'list' ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-700'"
                            @click="domainMode = 'list'">
                            Select from list
                        </button>
                        <button type="button"
                            class="px-3 py-2 text-sm font-medium border-b-2"
                            :class="domainMode === 'bulk' ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-700'"
                    ...
                    <div class="flex flex-wrap items-end gap-3 mb-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
                        ...
                        <input type="number" id="select-n-count" min="1" placeholder="e.g. 57"
                            class="w-24 rounded-lg border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                        <span class="text-sm text-slate-600">domains</span>
                        <select id="select-n-mode" class="rounded-lg border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                            <option value="top">from top</option>
                            <option value="bottom">from bottom</option>
                            <option value="random">random</option>
                        </select>
                        <button type="button" id="select-n-btn" class="px-3 py-1.5 bg-emerald-600 text-white text-sm rounded-lg hover:bg-emerald-700">
                            Apply
                        </button>
                        <span class="text-xs text-slate-500" id="domain-total-hint">({{ count($domains ?? []) }} total)</span>
                    </div>
                    </div>
                    ...
                    <div class="mb-4">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <button type="button" id="bulk-domain-select-all"
                        ...
                        <textarea id="bulk-domain-input" rows="6" placeholder="example.com&#10;anotherdomain.com"
                            class="w-full rounded-lg border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm"></textarea>
                        <p class="text-xs text-slate-500 mt-1">Paste one domain per line. We will select matching domains from the list below.
                            Domains not present in your account will be ignored.</p>
                        <p id="bulk-domain-missing" class="hidden text-xs text-amber-600 mt-1"></p>
                    ...
                    <!-- Domain checklist (always visible for review) -->
                    <div class="border border-slate-200 rounded-lg p-4 max-h-64 overflow-y-auto w-full">
                        <label class="flex items-center gap-2 p-2 hover:bg-slate-50 rounded cursor-pointer" id="select-all-label">
                            <input type="checkbox" id="select-all-cb" class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500">
                            Select All
                        </label>
                        <div class="mt-2 space-y-1" id="domain-list">
                ...
                <div x-show="step === 3" x-transition:enter="transition ease-out duration-200"
                    ...
                    {{-- Manual Links: Keywords + URLs (one per line, matched by sequence) --}}
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea name="links_bulk" id="links_bulk" rows="12" placeholder="https://example.com/page1&#10;https://example.com/page2&#10;https://example.com/page3"
                                    class="w-full rounded-lg border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 font-mono text-sm"></textarea>
...
@section('scripts')
    <script>
        (function() {
            ...
            // Domain selection: Select N from top/bottom/random
            ...
            // Bulk select by pasted domain names (must match existing domains in DB)
            ...
            const bulkDomainMissing = document.getElementById('bulk-domain-missing');
            const bulkDomainSelectAll = document.getElementById('bulk-domain-select-all');
            ...
            function applyBulkDomains() {
                if (!bulkDomainInput) return;
                const lines = (bulkDomainInput.value || '').split(/\r?\n/).map(l => normalizeDomain(l)).filter(Boolean);
                if (lines.length === 0) {
                    return;
                }
                ...
                if (bulkDomainMissing) {
                    if (missing.length) {
                        bulkDomainMissing.textContent = 'These domains were not found in your account and were ignored: ' + missing.slice(0, 5).join(', ') + (missing.length > 5 ? ' +' + (missing.length - 5) + ' more' : '');
                        bulkDomainMissing.classList.remove('hidden');
                    } else {
                        bulkDomainMissing.textContent = '';
                        bulkDomainMissing.classList.add('hidden');
                    }
                }
            }

            if (bulkDomainApply) {
                bulkDomainApply.addEventListener('click', applyBulkDomains);
            }

            if (bulkDomainSelectAll) {
                bulkDomainSelectAll.addEventListener('click', function() {
            ...
            form?.addEventListener('submit', function(e) {
                ...
                const domainIds = [...form.querySelectorAll('input[name="domain_ids[]"]:checked')].map(cb => cb.value);
                ...
                // Don't append domain_ids - checkboxes already submit them; appending would duplicate
                form.submit();
            });
        })();
    </script>
