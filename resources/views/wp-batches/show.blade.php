...
@section('title', $wpBatch->name ?? 'WP Batch Detail')
@section('page-title', $wpBatch->name ?? 'WP Batch Detail')

@section('content')
<div class="page-enter">
    <div class="mb-6">
        <a href="{{ route('wp-batches.index') }}" class="text-sm text-slate-500 hover:text-sky-600 mb-2 inline-block">← Back to WP Batches</a>
        <div class="flex flex-wrap items-center gap-4">
            <h2 class="text-2xl font-bold text-slate-800">{{ $wpBatch->name ?? 'Untitled WP Batch' }}</h2>
            @php
                $statusClasses = [
        ...
        @if($wpBatch->description ?? null)
            <p class="text-slate-500 mt-1">{{ $wpBatch->description }}</p>
        @endif
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            <p class="text-sm text-slate-500">Total Links</p>
            <p class="text-2xl font-bold text-slate-800 mt-1">{{ $wpBatch->total_links ?? 0 }}</p>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            <p class="text-sm text-slate-500">Total Sites</p>
            <p class="text-2xl font-bold text-slate-800 mt-1">{{ $wpBatch->total_domains ?? 0 }}</p>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            <p class="text-sm text-slate-500">Success</p>
            <p class="text-2xl font-bold text-emerald-600 mt-1">{{ $wpBatch->success_count ?? 0 }}</p>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            <p class="text-sm text-slate-500">Failed</p>
            <p class="text-2xl font-bold text-red-600 mt-1">{{ $wpBatch->failed_count ?? 0 }}</p>
        </div>
    </div>
    @if(($wpBatch->status ?? '') === 'deleting')
        <div class="bg-sky-50 border border-sky-200 rounded-lg p-4 mb-6 flex items-center gap-3">
            ...
            <span class="text-sky-800 font-medium">Deleting from remote sites… Unresponsive sites are skipped and processing continues on live sites.</span>
        </div>
    @endif
    @if(in_array($wpBatch->status ?? '', ['semi_deleted', 'delete_failed'], true))
        <div class="bg-violet-50 border border-violet-200 rounded-lg p-4 mb-6">
            <p class="text-violet-900 font-medium">{{ ($wpBatch->status ?? '') === 'semi_deleted' ? 'Semi deleted' : 'Delete incomplete' }}</p>
            <p class="text-sm text-violet-800 mt-1">Remote deletion finished for responsive sites. This batch now only lists sites where links could not be removed. Delete the batch again to retry remaining sites.</p>
        </div>
    @endif
    ...
    @php
        ...
        $siteFilterCounts = [
            'all' => count($siteStatsList),
            'success' => collect($siteStatsList)->filter(fn ($r) => ($r['success'] ?? 0) > 0)->count(),
            'pending' => collect($siteStatsList)->filter(fn ($r) => ($r['pending'] ?? 0) > 0)->count(),
            'failed' => collect($siteStatsList)->filter(fn ($r) => ($r['failed'] ?? 0) > 0)->count(),
        ];
    @endphp
        ...
        <div class="px-4 sm:px-6 py-5 border-b border-slate-200">
            <div class="flex flex-col gap-5">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-800">Site Progress</h3>
                        <p class="text-sm text-slate-500 mt-0.5">Remove offline or failing sites without stopping the batch on live sites.</p>
                    </div>
                    <div class="flex flex-wrap items-center gap-2 lg:justify-end">
                        ...
                        @endif
                        @if(($problemSiteCount ?? 0) > 0)
                            <form method="POST" action="{{ route('wp-batches.domains.remove-problem', $wpBatch) }}" class="inline" onsubmit="return confirm('Remove {{ $problemSiteCount }} problem site(s) from this batch? Pending/failed work on those sites will stop. Links already posted will be cleaned remotely when possible.');">
                                @csrf
                                <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-red-200 bg-red-50 text-red-700 hover:bg-red-100 transition-colors">
                                    ...
                                    Remove problem sites ({{ $problemSiteCount }})
                                </button>
                            </form>
                        @endif
                        <a id="batch-sites-download" href="{{ route('wp-batches.export-domains', $wpBatch) }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 transition-colors">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-4 border-t border-slate-100">
                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                        ...
                        <div class="inline-flex flex-wrap rounded-xl border border-slate-200 bg-slate-100/80 p-1 gap-1" role="tablist" aria-label="Filter sites by status">
                            <button type="button" data-site-filter="all" aria-selected="true" class="site-filter-btn px-4 py-2 text-sm font-semibold rounded-lg transition-all duration-200 bg-slate-700 text-white shadow-sm">
                                All <span class="site-filter-count tabular-nums ml-0.5 opacity-80">{{ number_format($siteFilterCounts['all']) }}</span>
                            </button>
                            <button type="button" data-site-filter="success" aria-selected="false" class="site-filter-btn px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 text-slate-600 hover:text-emerald-700 hover:bg-white/60">
                                Success <span class="site-filter-count tabular-nums ml-0.5 opacity-70">{{ number_format($siteFilterCounts['success']) }}</span>
                            </button>
                            <button type="button" data-site-filter="pending" aria-selected="false" class="site-filter-btn px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 text-slate-600 hover:text-slate-800 hover:bg-white/60">
                                Pending <span class="site-filter-count tabular-nums ml-0.5 opacity-70">{{ number_format($siteFilterCounts['pending']) }}</span>
                            </button>
                            <button type="button" data-site-filter="failed" aria-selected="false" class="site-filter-btn px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 text-slate-600 hover:text-red-700 hover:bg-white/60">
                                Failed <span class="site-filter-count tabular-nums ml-0.5 opacity-70">{{ number_format($siteFilterCounts['failed']) }}</span>
                            </button>
                        </div>
                    </div>
                    <p id="site-filter-count" class="text-xs text-slate-500 sm:text-right min-h-[1rem]"></p>
                </div>
            </div>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Site</th>
                ...
                <tbody class="divide-y divide-slate-200" id="site-stats-body">
                    @forelse($siteStatsList as $row)
                        <tr class="hover:bg-slate-50/50 transition-colors site-stat-row"
                            ...
                            <td class="px-6 py-4 text-right">
                                ...
                                <a href="{{ route('wp-batches.show-domain', [$wpBatch, $row['wp_site_id'] ?? 0]) }}" class="text-sky-600 hover:text-sky-700 text-sm font-medium">View</a>
                                <form method="POST" action="{{ route('wp-batches.domains.destroy', [$wpBatch, $row['wp_site_id'] ?? 0]) }}" class="inline" onsubmit="return confirm('Remove this site from the batch? Pending posts to this site will stop.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-1.5 rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-600 transition-colors" title="Remove site from batch">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                    </button>
                                </form>
                                </div>
                    ...
                    @empty
                        <tr id="site-stats-empty">
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                                No site statistics yet. Start posting to see results.
                            </td>
                        </tr>
                    @endforelse
                    <tr id="site-filter-empty" class="hidden">
                        <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                    ...
                    @forelse($links ?? [] as $i => $link)
                        <tr class="hover:bg-slate-50/50 transition-colors link-row" data-url="{{ strtolower($link->url) }}" data-keyword="{{ strtolower($link->keyword) }}">
                            ...
                            <td class="px-6 py-3 text-right">
                                <form method="POST" action="{{ route('wp-batches.links.destroy', [$wpBatch, $link]) }}" class="inline" onsubmit="return confirm('Delete this link from the batch and remove it from all remote sites?');">
                                    @csrf
                                    @method('DELETE')
...
(function() {
    ...
    const siteRows = document.querySelectorAll('.site-stat-row');
    const filterEmptyRow = document.getElementById('site-filter-empty');
    const filterCountEl = document.getElementById('site-filter-count');
    const downloadLink = document.getElementById('batch-sites-download');
    const downloadBaseUrl = downloadLink ? downloadLink.getAttribute('href') : '';
    let activeFilter = 'all';

    const filterBaseClass = 'site-filter-btn px-4 py-2 text-sm rounded-lg transition-all duration-200';
    const activeClasses = {
    ...
    function applyFilterButtonState(btn, key, isActive) {
        ...
        const countEl = btn.querySelector('.site-filter-count');
        if (countEl) {
            countEl.className = 'site-filter-count tabular-nums ml-0.5 ' + (isActive ? 'opacity-80' : 'opacity-70');
        }
    }
    ...
    function updateSiteFilter(filter) {
        activeFilter = filter;
        let visible = 0;

        filterButtons.forEach(function(btn) {
            const key = btn.getAttribute('data-site-filter');
        ...
        if (filterEmptyRow) {
            filterEmptyRow.classList.toggle('hidden', visible > 0 || siteRows.length === 0);
        }
        if (filterCountEl && siteRows.length > 0) {
            const countColors = {
                all: 'text-slate-600',
            ...
            filterCountEl.textContent = filter === 'all'
                ? ''
                : 'Showing ' + visible.toLocaleString() + ' of ' + siteRows.length.toLocaleString() + ' sites';
        }
    ...
    filterButtons.forEach(function(btn) {
        btn.addEventListener('click', function() {
            updateSiteFilter(btn.getAttribute('data-site-filter') || 'all');
        });
    });
